<?xml version='1.0'?>
<!--############################################################################# 
|	$Id: lists.mod.xsl,v 1.1 2005/12/05 08:19:40 rpopov Exp $
|- #############################################################################
|	$Author: rpopov $
|														
|   PURPOSE:
...
<xsl:stylesheet 
	...
	<xsl:text>% \null is a trick to induce different typesetting decisions&#10;</xsl:text>
	<xsl:text>\item[</xsl:text><xsl:apply-templates select="term"/><xsl:text>]\null{}</xsl:text>
	<xsl:apply-templates select="listitem"/>
    </xsl:template>
