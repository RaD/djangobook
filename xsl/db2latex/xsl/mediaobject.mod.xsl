<?xml version='1.0'?>
<!--############################################################################# 
|	$Id: mediaobject.mod.xsl,v 1.1 2005/12/05 08:19:40 rpopov Exp $
|- #############################################################################
|	$Author: rpopov $												
|														
...
<xsl:stylesheet 
	...
	<xsl:variable name="imagemediascnt" select="count(../../..//mediaobject[imageobject])"/>
	...
	<xsl:if test="$imagemediascnt &gt; 1 and $latex.use.subfigure='1' and count(ancestor::figure) &gt; 0">
