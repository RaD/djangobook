<?xml version='1.0'?>
<!--############################################################################# 
|	$Id: figure.mod.xsl,v 1.1 2005/12/05 08:19:40 rpopov Exp $
|- #############################################################################
|	$Author: rpopov $												
|														
...
<xsl:stylesheet 
	...
	<refdescription>
		...
		<itemizedlist>
			<listitem><para>The <literal>figure.title</literal> can be typset in italics by specifying <literal>$latex.figure.title.style</literal> (<literal>\textit</literal> would be common).</para></listitem>
...
<!-- Auxiliary template to output a figure caption.-->
<xsl:template name="aux.informalfigure.caption">
<xsl:if test="count(child::mediaobject/caption)=1">
...
<xsl:template match="informalfigure">
	<xsl:call-template name="map.begin"/>
	<xsl:apply-templates/>
	<xsl:call-template name="aux.informalfigure.caption" />
	<xsl:call-template name="map.end"/>
</xsl:template>
