<?xml version='1.0'?>
<!--############################################################################# 
|	$Id: xref.mod.xsl,v 1.1 2005/12/05 08:19:41 rpopov Exp $
|- #############################################################################
|	$Author: rpopov $
|														
...
<!--############################################################################# -->
    ...
    <xsl:template match="informalfigure|informalexample|informaltable|informalequation" mode="xref-to">
	<xsl:param name="target" select="."/>
	<xsl:call-template name="cross-reference">
	    <xsl:with-param name="target" select="$target"/>
	    <xsl:with-param name="refelem" select="'page'"/>
	</xsl:call-template>
    </xsl:template>
  ...
  <refdescription>
  	<formalpara>
		...
		<itemizedlist>
			...
			<listitem><simpara><xref linkend="param.ulink.protocols.relaxed"/></simpara></listitem>
...
<!--############################################################################# -->
	...
	<xsl:choose>
		...
		<xsl:when test="$ulink.protocols.relaxed='1' and (substring-after($url,':')=$content or substring-after($url,'://')=$content)">
			<xsl:call-template name="generate.typeset.url">
				<xsl:with-param name="hyphenation" select="$hyphenation"/>
...
<!--############################################################################# -->
	...
	<doc:template name="generate.typeset.url">
	</doc:template>
